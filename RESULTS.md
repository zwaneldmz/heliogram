...
**Provenance:** Python 3.12.3; numpy 2.1.2; Pillow 11.0.0; reedsolo 1.7.0; platform: Linux-6.8.0-87-generic-x86_64-with-glibc2.39
...
- **Other text encodings (is base64 even the right bar?):** MEASURED (Qwen/Qwen2.5-VL-7B-Instruct, transformers==5.13.0): `ascii85` 8.374 bits/token (1.3084 chars/token), `base85` 8.178 bits/token (1.2779 chars/token), `base64` 8.096 bits/token (1.3498 chars/token), `hex` 4.534 bits/token (1.1334 chars/token). The strongest (`ascii85`) is the honest text-context economic bar; the Bar A verdicts above are computed against base64 and carry the qualifier stated there.

...
| 4 | 2 | 1024 | 8 | 6.687 | yes | no | 6.687 | resize_3pct | no | no |
| 4 | 2 | 4096 | 8 | 6.784 | yes | no | 6.784 | resize_3pct | no | no |
| 4 | 2 | 16384 | 8 | 6.933 | yes | no | 6.933 | resize_3pct | no | no |
...
| 16 | 2 | 48 | 16 | 6.000 | yes | no | 0.000 | combined | no | no |
...
| 128 | 1 | 16384 | 7 | 6.066 | yes | no | 0.000 | jpeg_q85 | no | no |
...
| 256 | 1 | 4096 | 8 | 6.400 | yes | no | 0.000 | jpeg_q85 | no | no |
| 256 | 1 | 16384 | 8 | 6.827 | yes | no | 0.000 | jpeg_q85 | no | no |
...
- palette=4, subpatch=2, payload_size=1024B -- clean 6.687 bits/patch (worst-corruption: 6.687, `resize_3pct`, does NOT clear Bar A under that corruption)
- palette=4, subpatch=2, payload_size=4096B -- clean 6.784 bits/patch (worst-corruption: 6.784, `resize_3pct`, does NOT clear Bar A under that corruption)
- palette=4, subpatch=2, payload_size=16384B -- clean 6.933 bits/patch (worst-corruption: 6.933, `resize_3pct`, does NOT clear Bar A under that corruption)
...
- palette=16, subpatch=2, payload_size=48B -- clean 6.000 bits/patch (worst-corruption: 0.000, `combined`, does NOT clear Bar A under that corruption)
...
- palette=128, subpatch=1, payload_size=16384B -- clean 6.066 bits/patch (worst-corruption: 0.000, `jpeg_q85`, does NOT clear Bar A under that corruption)
...
- palette=256, subpatch=1, payload_size=4096B -- clean 6.400 bits/patch (worst-corruption: 0.000, `jpeg_q85`, does NOT clear Bar A under that corruption)
- palette=256, subpatch=1, payload_size=16384B -- clean 6.827 bits/patch (worst-corruption: 0.000, `jpeg_q85`, does NOT clear Bar A under that corruption)
...
Every Gate #1 (Bar B) clearing config has `subpatch>1` -- the unverified pixel-decoder geometric ceiling regime. **No `subpatch=1` (VLM-meaningful) config clears Gate #1 at any tested payload size.** This is not just an unlucky corruption result: for `subpatch=1` the raw per-symbol ceiling is `log2(palette)`, which for the largest palette tested (256) is only 8 bits/patch -- already below the 8-bit Bar B *before* Reed-Solomon/calibration overhead is even subtracted. No amount of payload-size amortization can close that gap for `subpatch=1`; only the geometric `subpatch>1` regime can mathematically reach Bar B, and whether a real VLM can realize that regime is exactly the open question Phase 2 exists to answer. **Bar A tells a different story, though:** 25 config(s) beat the real economic bar clean (see the list above), including subpatch=1 configs -- see the "Token crossover" section below for what that means in tokens, and the mandatory P=128/256 corruption caveat above for what it does not yet mean.

## Baselines

- **base64 in text context:** ~6.0 bits/token (analytic: base64 alphabet size 64 -> log2(64)=6 bits/char; ~1 char/token for typical BPE tokenizers on base64 streams. Pass a real tokenizer for a measured value.). Source: ANALYTIC assumption (no measured-tokenizer baseline file found in this run -- see `heliogram.baselines.load_measured_baseline`) (see `_resolve_base64_baseline`). **Every Bar A ('beats base64 clean?') verdict in the Headline table above and every `GATE_BITS_PER_PATCH`/`BASE64_BITS_PER_TOKEN` comparison anywhere in this file is computed directly against THIS number.** Bar C's `token_ratio`/`heliogram_cheaper` in the "Token crossover" section below is DIFFERENT: it is computed from a separate, fixed closed-form estimate (`base64_token_est = ceil(payload/3)*4`, an implicit ~1-base64-char/token assumption) that does **not** currently update from this resolved baseline even when a measured one is available -- so if the source above is ANALYTIC, both Bar A and Bar C rest on the same 6-bits/token assumption; if it is MEASURED, only Bar A reflects the measurement and Bar C still rests on the analytic ~1-char/token estimate. This asymmetry is a known limitation, not an oversight papered over here.
...
| 128 | 1 | 48 | 256 | 64 | 4.000 | no | 1.00 | 1.00 |
...
|---|---|---|---|---|---|---|---|---|
| 2 | 1 | 48 | 729 | 11.391 | no | 196 | 3.062 | no |
| 2 | 1 | 1024 | 9702 | 7.092 | no | 2450 | 1.791 | no |
| 2 | 1 | 4096 | 38025 | 6.959 | no | 9604 | 1.758 | no |
| 2 | 1 | 16384 | 150544 | 6.891 | no | 37636 | 1.723 | no |
| 2 | 2 | 48 | 196 | 3.062 | no | 49 | 0.766 | **YES** |
| 2 | 2 | 1024 | 2450 | 1.791 | no | 625 | 0.457 | **YES** |
| 2 | 2 | 4096 | 9604 | 1.758 | no | 2401 | 0.439 | **YES** |
| 2 | 2 | 16384 | 37830 | 1.732 | no | 9506 | 0.435 | **YES** |
| 4 | 1 | 48 | 361 | 5.641 | no | 100 | 1.562 | no |
| 4 | 1 | 1024 | 4830 | 3.531 | no | 1225 | 0.895 | **YES** |
| 4 | 1 | 4096 | 19044 | 3.485 | no | 4761 | 0.871 | **YES** |
| 4 | 1 | 16384 | 75350 | 3.449 | no | 18906 | 0.865 | **YES** |
| 4 | 2 | 48 | 100 | 1.562 | no | 25 | 0.391 | **YES** |
| 4 | 2 | 1024 | 1225 | 0.895 | yes | 324 | 0.237 | **YES** |
| 4 | 2 | 4096 | 4830 | 0.884 | yes | 1225 | 0.224 | **YES** |
| 4 | 2 | 16384 | 18906 | 0.865 | yes | 4761 | 0.218 | **YES** |
| 8 | 1 | 48 | 256 | 4.000 | no | 64 | 1.000 | no |
| 8 | 1 | 1024 | 3249 | 2.375 | no | 841 | 0.615 | **YES** |
| 8 | 1 | 4096 | 12769 | 2.337 | no | 3249 | 0.595 | **YES** |
| 8 | 1 | 16384 | 50400 | 2.307 | no | 12656 | 0.579 | **YES** |
| 8 | 2 | 48 | 72 | 1.125 | no | 20 | 0.312 | **YES** |
| 8 | 2 | 1024 | 841 | 0.615 | yes | 225 | 0.164 | **YES** |
| 8 | 2 | 4096 | 3249 | 0.595 | yes | 841 | 0.154 | **YES** |
| 8 | 2 | 16384 | 12656 | 0.579 | yes | 3192 | 0.146 | **YES** |
| 16 | 1 | 48 | 192 | 3.000 | no | 48 | 0.750 | **YES** |
| 16 | 1 | 1024 | 2450 | 1.791 | no | 625 | 0.457 | **YES** |
| 16 | 1 | 4096 | 9604 | 1.758 | no | 2401 | 0.439 | **YES** |
| 16 | 1 | 16384 | 37830 | 1.732 | no | 9506 | 0.435 | **YES** |
| 16 | 2 | 48 | 64 | 1.000 | no | 16 | 0.250 | **YES** |
| 16 | 2 | 1024 | 625 | 0.457 | yes | 169 | 0.124 | **YES** |
| 16 | 2 | 4096 | 2450 | 0.448 | yes | 625 | 0.114 | **YES** |
| 16 | 2 | 16384 | 9506 | 0.435 | yes | 2401 | 0.110 | **YES** |
| 32 | 1 | 48 | 192 | 3.000 | no | 48 | 0.750 | **YES** |
| 32 | 1 | 1024 | 1980 | 1.447 | no | 506 | 0.370 | **YES** |
| 32 | 1 | 4096 | 7656 | 1.401 | no | 1936 | 0.354 | **YES** |
| 32 | 1 | 16384 | 30276 | 1.386 | no | 7569 | 0.346 | **YES** |
| 32 | 2 | 48 | 96 | 1.500 | no | 32 | 0.500 | **YES** |
| 32 | 2 | 1024 | 512 | 0.374 | yes | 128 | 0.094 | **YES** |
| 32 | 2 | 4096 | 1936 | 0.354 | yes | 484 | 0.089 | **YES** |
| 32 | 2 | 16384 | 7656 | 0.350 | yes | 1936 | 0.089 | **YES** |
| 64 | 1 | 48 | 192 | 3.000 | no | 64 | 1.000 | no |
| 64 | 1 | 1024 | 1664 | 1.216 | no | 416 | 0.304 | **YES** |
| 64 | 1 | 4096 | 6400 | 1.171 | no | 1600 | 0.293 | **YES** |
| 64 | 1 | 16384 | 25281 | 1.157 | no | 6400 | 0.293 | **YES** |
| 64 | 2 | 48 | 128 | 2.000 | no | 32 | 0.500 | **YES** |
| 64 | 2 | 1024 | 512 | 0.374 | yes | 128 | 0.094 | **YES** |
| 64 | 2 | 4096 | 1664 | 0.305 | yes | 416 | 0.076 | **YES** |
| 64 | 2 | 16384 | 6400 | 0.293 | yes | 1600 | 0.073 | **YES** |
| 128 | 1 | 48 | 256 | 4.000 | no | 64 | 1.000 | no |
| 128 | 1 | 1024 | 1536 | 1.123 | no | 384 | 0.281 | **YES** |
| 128 | 1 | 4096 | 5632 | 1.031 | no | 1408 | 0.258 | **YES** |
| 128 | 1 | 16384 | 21609 | 0.989 | yes | 5476 | 0.251 | **YES** |
| 128 | 2 | 48 | 256 | 4.000 | no | 64 | 1.000 | no |
| 128 | 2 | 1024 | 512 | 0.374 | yes | 128 | 0.094 | **YES** |
| 128 | 2 | 4096 | 1536 | 0.281 | yes | 384 | 0.070 | **YES** |
| 128 | 2 | 16384 | 5504 | 0.252 | yes | 1408 | 0.064 | **YES** |
| 256 | 1 | 48 | 512 | 8.000 | no | 128 | 2.000 | no |
| 256 | 1 | 1024 | 1536 | 1.123 | no | 384 | 0.281 | **YES** |
| 256 | 1 | 4096 | 5120 | 0.937 | yes | 1280 | 0.234 | **YES** |
| 256 | 1 | 16384 | 19200 | 0.879 | yes | 4864 | 0.223 | **YES** |
| 256 | 2 | 48 | 512 | 8.000 | no | 128 | 2.000 | no |
| 256 | 2 | 1024 | 768 | 0.561 | yes | 256 | 0.187 | **YES** |
| 256 | 2 | 4096 | 1536 | 0.281 | yes | 384 | 0.070 | **YES** |
| 256 | 2 | 16384 | 5120 | 0.234 | yes | 1280 | 0.059 | **YES** |
...
- palette=2: per-patch no crossover found in the exact scan up to 65536B (lowest ratio observed: 6.870 at 64219B); LM-token (UNVERIFIED, see caveat above) no crossover found in the exact scan up to 65536B (lowest ratio observed: 1.718 at 60421B)
- palette=4: per-patch no crossover found in the exact scan up to 65536B (lowest ratio observed: 3.437 at 64219B); LM-token (UNVERIFIED, see caveat above) crosses at 148B (exact) -- WOBBLES: recrosses back to >= 1.0 at 153B (and 6 more time(s) within the scanned range) -- NOT a stable one-way threshold, see exact_crossover_payload_size's docstring
- palette=8: per-patch no crossover found in the exact scan up to 65536B (lowest ratio observed: 2.292 at 63772B); LM-token (UNVERIFIED, see caveat above) crosses at 49B (exact) -- WOBBLES: recrosses back to >= 1.0 at 54B (and 1 more time(s) within the scanned range) -- NOT a stable one-way threshold, see exact_crossover_payload_size's docstring
- palette=16: per-patch no crossover found in the exact scan up to 65536B (lowest ratio observed: 1.720 at 65308B); LM-token (UNVERIFIED, see caveat above) crosses at 31B (exact) -- WOBBLES: recrosses back to >= 1.0 at 36B (and 0 more time(s) within the scanned range) -- NOT a stable one-way threshold, see exact_crossover_payload_size's docstring
- palette=32: per-patch no crossover found in the exact scan up to 65536B (lowest ratio observed: 1.376 at 63550B); LM-token (UNVERIFIED, see caveat above) crosses at 37B (exact)
- palette=64: per-patch no crossover found in the exact scan up to 65536B (lowest ratio observed: 1.147 at 64660B); LM-token (UNVERIFIED, see caveat above) crosses at 49B (exact)
- palette=128: per-patch crosses at 5569B (exact) -- WOBBLES: recrosses back to >= 1.0 at 5571B (and 48 more time(s) within the scanned range) -- NOT a stable one-way threshold, see exact_crossover_payload_size's docstring; LM-token (UNVERIFIED, see caveat above) crosses at 49B (exact) -- WOBBLES: recrosses back to >= 1.0 at 76B (and 0 more time(s) within the scanned range) -- NOT a stable one-way threshold, see exact_crossover_payload_size's docstring
- palette=256: per-patch crosses at 1537B (exact) -- WOBBLES: recrosses back to >= 1.0 at 1557B (and 5 more time(s) within the scanned range) -- NOT a stable one-way threshold, see exact_crossover_payload_size's docstring; LM-token (UNVERIFIED, see caveat above) crosses at 97B (exact)

**subpatch=2 (PIXEL-DECODER GEOMETRIC CEILING ONLY -- not a VLM capability claim, see caveat above):**

- palette=2: per-patch no crossover found in the exact scan up to 65536B (lowest ratio observed: 1.720 at 65308B); LM-token (UNVERIFIED, see caveat above) crosses at 28B (exact) -- WOBBLES: recrosses back to >= 1.0 at 30B (and 1 more time(s) within the scanned range) -- NOT a stable one-way threshold, see exact_crossover_payload_size's docstring
- palette=4: per-patch crosses at 157B (exact) -- WOBBLES: recrosses back to >= 1.0 at 160B (and 7 more time(s) within the scanned range) -- NOT a stable one-way threshold, see exact_crossover_payload_size's docstring; LM-token (UNVERIFIED, see caveat above) crosses at 13B (exact)
- palette=8: per-patch crosses at 55B (exact) -- WOBBLES: recrosses back to >= 1.0 at 60B (and 0 more time(s) within the scanned range) -- NOT a stable one-way threshold, see exact_crossover_payload_size's docstring; LM-token (UNVERIFIED, see caveat above) crosses at 10B (exact)
- palette=16: per-patch crosses at 49B (exact) -- WOBBLES: recrosses back to >= 1.0 at 60B (and 0 more time(s) within the scanned range) -- NOT a stable one-way threshold, see exact_crossover_payload_size's docstring; LM-token (UNVERIFIED, see caveat above) crosses at 13B (exact)
- palette=32: per-patch crosses at 73B (exact); LM-token (UNVERIFIED, see caveat above) crosses at 13B (exact)
- palette=64: per-patch crosses at 97B (exact); LM-token (UNVERIFIED, see caveat above) crosses at 25B (exact)
- palette=128: per-patch crosses at 193B (exact); LM-token (UNVERIFIED, see caveat above) crosses at 49B (exact)
- palette=256: per-patch crosses at 385B (exact); LM-token (UNVERIFIED, see caveat above) crosses at 97B (exact)

### Token-crossover verdict

At `subpatch=1` (the only VLM-meaningful regime), the following palette(s) cross below base64 token count, PER-PATCH accounting, within the exact scanned range: palette=128 at 5569B (exact), palette=256 at 1537B (exact). This is the project's actual, currently-measured benefit claim: for a large enough payload, encoding it as a heliogram grid costs fewer total patches than base64-ing it into text tokens, and (per Bar A in the Headline section) does so at a bits/patch density that also beats plain base64 text, and is bit-exact on a successful decode (Reed-Solomon verified). **This is a clean-channel, token-accounting result only** -- see the mandatory P=128/256 corruption caveat in the Headline section above: the pixel decoder cannot currently realize this benefit end to end under `jpeg_q70` at these same palettes. The open question is purely whether a fine-tuned VLM reader can, which is Phase 2 and is not measured here. The LM-token (2x2 merger) accounting above crosses at smaller payload sizes still, but carries the additional, separate, UNVERIFIED assumption that a merged embedding can be read at 4x the per-patch symbol density -- see that subsection's caveat; it is not folded into this verdict.
